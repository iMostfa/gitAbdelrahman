...
class gitNetworkTestWithResponse: QuickSpec  {
  ...
  """
      ...
      override func spec() {
        ...
        guard let gitUrl = URL(string: "https://api.github.com/users/hoang-tran/repos") else { return }
        ...
        describe("requestUserWithName") {
          context("success") {
            it("returns GitHubUserData") {
                ...
                GitDataDownloader.shared.downloadRepos(for: "hoang-tran") { (items,error) in
                ...
                expect(self.testItems).toEventuallyNot(beNil())
                expect(self.testItems?[0].name) == "Animate"
                 expect(self.testItems?[0].language) == "Swift"
